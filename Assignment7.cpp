...
/*
Assignment no 7
Problem statement:-Implementation of Cohen Sutherland line clipping algorithm
...
void LineClip(double X1, double Y1, double X2, double Y2)
{
	...
	do
	{
		if(outcode1==0 && outcode2==0)	//completely visible line
		{
			accept=1;
			done=1;
		}
		else if((outcode1 & outcode2)!=0)	//completely invisible line
		{
			done=1;
		}
		else
		{
			...
			if(outcode1==0)	//if one endpoint is completely inside the window
			temp=outcode2;
			else
			temp=outcode1;
